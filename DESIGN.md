...
- WebDAV 目录内容层复用生产文件列表与状态卡，来源、请求取消、缓存、本机进度和播放准备仍由生产宿主持有。
- 路径、收藏、文件行、播放和文件信息保持至少 48dp 实际触控目标；长路径与大字号通过滚动和自然增高保持可读。
- 继续观看明确标注“仅本机”，卡片宽度随字号增长；错误态显式重试，空目录不与错误或缓存状态叠加。
- 排序/显示面板保持 WebDAV 自己的底部面板宿主，只复用共享选择行和开关语义，不与视频播放器窗内面板混用。

### 设置与服务器表单

- `SettingsRow` 最低 64dp,值的位置由真实标题/值测量、图标和箭头预算决定;大字体或长值放在标题下方,不再使用字符串长度阈值。
- 设置主页按「连接 / 体验与播放 / 数据与应用」分组,组内入口行与分隔线保持同一直径(0.5 alpha);模块显示使用一组安静表面与分隔线,说明「至少保留一个」和「隐藏不删除数据」;开关仍由持久化结果驱动。
- 服务器列表保留 Radio 选择语义与溢出菜单(编辑/测试/删除),测试状态行使用实色 `onSurfaceVariant`,失败用 `error`;未添加来源使用紧凑状态卡。
- 设置选择对话框选中行使用 `primaryContainer` 背景 + `onPrimaryContainer` 文案强调,与全应用选择语言一致。
- 服务器表单分为连接信息与身份验证;复用 `ConfigTextField` 的字体、边框、密码安全键盘、可见性按钮和支持说明。旧的单服务器 `Navidrome/Audiobook/VideoConfigCard` 已无生产引用,已从 `ConfigCards.kt` 移除。
