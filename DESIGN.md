...
- 封面/歌词主显示区同时暴露手势与语义路径:双击左半/右半区快退/快进 10 秒,单击切换封面与歌词;用户无测视障碍通过 TalkBack 自定义动作(「显示歌词/显示封面」「后退/前进 10 秒」)获得同等功能,不新增可见按钮。
- 播放器顶栏标题展示 heading 语义,与全应用页头规范一致。
- 迷你播放条的状态文字按语义选色:无状态默认元信息(onSurfaceVariant),缓冲等瞬时强调用 primary,错误用 error;播放/暂停与收起把手暴露 Button 角色,导航项图标不重复朗读(标签与选中语义承担)。
- 有声书睡眠定时器面板按预设与固定分钟去重:当前预选值仅由「使用预选:N 分钟」入口提供,固定分钟列表过滤掉该值,避免重复动作;预选选中语义与其他选择行一致。
