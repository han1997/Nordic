...
「设置 → 备份与恢复」可将逻辑数据手动备份到自己的 WebDAV，并可跨设备恢复：

- 备份内容：服务器连接与凭据、应用偏好、音乐播放历史、有声书书签与阅读位置、WebDAV 收藏与观看进度。不包含已下载音乐与各类缓存。
- 备份使用独立的 WebDAV 配置（地址、账号、密码、备份目录），与媒体来源互不影响；支持 Basic 与匿名认证，不支持仅 Digest 的服务，HTTP 需明确确认。
- 归档由你设置的备份密码加密（PBKDF2 + AES-GCM）。备份密码遗失后云端归档无法恢复，请妥善保管。
- 每次备份生成带 UTC 时间戳的新归档，云端自动保留最近 5 份；恢复前可按时间、创建版本与大小选择归档。
- 恢复为「完整覆盖」：确认后替换本机的对应数据，不与现有数据合并；已下载音乐与缓存不受影响。解密失败、格式不兼容或网络失败时本地数据零改动。
- 恢复成功后请退出并重新打开应用，以加载恢复后的数据。
