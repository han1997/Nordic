...
- ON_STOP lifecycle safety net: when the app is backgrounded during audiobook playback, push one immediate `syncProgress` (via `AudiobookPlaybackViewModel.syncNow()`) using the resume-aware baseline. Playback continues in the background — do NOT close the session or stop playback on ON_STOP (unlike video's close-on-stop semantics). Failures are silent (Log only).
